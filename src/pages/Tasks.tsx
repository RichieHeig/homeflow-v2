...
export default function Tasks() {
  ...
  useEffect(() => {
    loadData()
  }, [user, filter, selectedMemberFilter])

  const loadData = async () => {
    if (!user) return

    try {
      // Si on n'a pas encore de household, on le charge
      if (!household) {
      ...
      // Récupérer les tâches (en utilisant household existant ou celui qu'on vient de charger)
      const householdId = household?.id || (await supabase
        .from('members')
        .select('household_id')
        .eq('id', user.id)
        .single()).data?.household_id
